...
// asyncの中でawaitを指定するとこのなかの処理が終わってから次の処理を行うようになる
const searchCustomers = async () => {
    try {
        // axiosを使用して検索の時だけページの読み込みが行われるようにする
...
// 引数にeを指定すると渡されたものをe.kanaのように取得できる
const setCustomer = e => {
    search.value = e.kana
...
// クリックするたびにisShowのステータスをtrueにしたりfalseにしたりする。
...
<template>
    ...
    <button @click="searchCustomers" type="button" data-micromodal-trigger="modal-1" class="flex mx-auto mt-6 text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg">検索する</button>
</template>
