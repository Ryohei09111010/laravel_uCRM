...
<template>
        ...
        <table class="table-auto w-full text-left whitespace-no-wrap">
            ...
            <tbody>
                <tr v-for="item in data.data" :key="item.data">
                    ...
                    <td class="border-b-2 border-gray-200 px-4 py-3">{{ item.total_radio }}</td>
                </tr>
            </tbody>
        </table>
    </div>
    <div v-if="data.type === 'rfm'" class="lg:w-2/3 w-full mx-auto overflow-auto">
        ...
        <h3 class="text-center mt-12 text-2xl my-4">RとFの集計表</h3>
        <table class="table-auto w-full text-left whitespace-no-wrap">
            <thead>
                <tr>
                    <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tl rounded-bl">Rランク</th>
